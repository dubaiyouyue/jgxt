<?php
	...
	$dtime=$_GET['d']+0;
	$mtime=$_GET['m']+0;
	$stmtall = $dbh->prepare('SELECT id from p where dtime=:dtime and mtime=:mtime');
	$stmtall->execute(array(':dtime' => $dtime,':mtime' => $mtime));
	//echo $stmtall->rowCount();
	$pageCount = ceil($stmtall->rowCount()/$m);
	
	
	$stmt = $dbh->prepare('SELECT * from p where dtime=:dtime and mtime=:mtime order by id desc limit :mp,:m');
...
$(".tcdPageCode").createPage({ 
  ...
  backFn:function(p){ 
   //单击回调方法，p是当前页码 
   location.href = 'assdm.php?d=<?php echo $dtime;?>&m=<?php echo $mtime;?>&p='+p;
  } 
 }); 
</script>
