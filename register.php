...
<P style="margin: 0px 35px 20px 45px;">
           <SPAN style="float: right;"><A style="color: green; margin-right: 10px;" 
href="/login.php">登录 </a>

<input type="button" style="background: rgb(0, 142, 173); padding: 7px 10px; border-radius: 4px; border: 1px solid rgb(26, 117, 152); border-image: none; color: rgb(255, 255, 255); font-weight: bold;"  value="注册" onclick="ajaxpost();" />
...
function ajaxpost(){
    ...
    $.ajax( {
        url:"/redb.php", 
        type: "post", 
        data:"yhm="+yhm+"&password="+password,
        //dataType: "json",
        //cache: false,
        //async: false,
        beforeSend: function(){
        ...
        success: function(datas){
            ...
            if(datas==1){
				layer.alert('数据异常！注册失败！',{title:'提示'});
				newsss();
				return false;
			}else if(datas==2){
				layer.alert('用户名已经被注册！注册失败！',{title:'提示'});newsss();
				return false;
			}else if(datas=='ok'){
				layer.alert('注册成功！请等待管理员审核！',{title:'提示'},function(){
					location.href = 'login.php';
				});newsss();
				return false;
			}else{
				layer.alert('服务器繁忙！注册失败！',{title:'提示'});newsss();
				return false;
			}
        }
    });
